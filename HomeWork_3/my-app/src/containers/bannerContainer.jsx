...
const BannerContainer = () => {
  ...
  const goToSlide = (index) => {
    if (!slidesData.length || index === currentIndex) return;
  ...
  useEffect(() => {
    const loadSlides = async () => {
      try {
        const response = await fetch('/api/slidesData.json');

        if (!response.ok) {
          throw new Error('Ошибка загрузки slidesData.json');
        }
    ...
    <div className="mt-3 slider--container">
      ...
      <div className="slider--nav__button--right">
        <button
          ...
          className="button--ripple slider--nav__buttons"
        >
